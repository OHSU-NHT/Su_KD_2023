...
# Import the 'KD DEGs.RData' file (supplemental data from original publication) into the R Studio environment

# 1. Convert Gene Symbols to ENTREZ IDs ------------------------------------------------------------------
markers <- DCT2.kd %>% rownames_to_column(var="SYMBOL")

ENTREZ_list <- bitr(geneID = rownames(DCT2.kd),
                    fromType = "SYMBOL",
                    toType = "ENTREZID",
                    OrgDb = "org.Mm.eg.db"
)
...
# 2. DCT2 KD up-regulated pathways ---------------------------------------------------------------------------

# Up-regulated Genes = markers with positive fold change values 
...
# Barplot of DCT2 KD up-regulated pathways
...
# 3. DCT2 KD down-regulated pathways ---------------------------------------------------------------------------

# Down-regulated Genes = markers with positive fold change values 
...
# Barplot of DCT2 KD down-regulated pathways
